...
<template>
  <div class="bg-background text-foreground">
    ...
    <nav class="fixed top-4 right-0 left-0 z-50">
      <div class="bg-background/80 border-border mx-auto max-w-7xl rounded-xl border-b pr-4 pl-8 backdrop-blur-md">
        <div class="flex h-16 items-center justify-between">
          <NuxtLink to="/" class="flex items-center gap-2">
            <span class="text-xl font-bold">{{ $t("landing_nav.app_name") }}</span>
          </NuxtLink>
          <!-- Desktop Menu -->
          ...
          <!-- Mobile Menu Button -->
          <button class="hover:bg-muted rounded-lg p-2 transition-colors md:hidden"
            :aria-label="$t('ui.toggle_menu')" @click="toggleMobileMenu">
            <Menu v-if="!mobileMenuOpen" :size="24" class="text-foreground" />
            <X v-else :size="24" class="text-foreground" />
          </button>
        </div>
      </div>
    </nav>
          ...
          <div class="flex h-full flex-col">
            ...
            <div class="border-border flex items-center justify-between border-b p-6">
              ...
              <div class="flex items-center gap-2">
                <ThemeToggle />
                <button class="hover:bg-muted rounded-lg p-2 transition-colors" :aria-label="$t('ui.close_menu')"
    ...
    <section class="bg-card text-card-foreground relative border-t pt-24 pb-12">
      <div class="relative z-10 mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <p class="text-muted-foreground mb-8">Join thousands of user using FullStackApp.</p>
      ...
      <footer class="relative z-10">
        <div class="mx-auto max-w-7xl px-4 pt-24 pb-12 sm:px-6 lg:px-8">
          <div class="mb-16 grid grid-cols-2 gap-12 md:grid-cols-4">
            ...
            <div>
              <h4 class="mb-6 text-xs font-bold tracking-widest uppercase">Social Media
              </h4>
              <ul class="space-y-4 text-sm font-semibold">
                <li>
                  <a href="#" class="text-muted-foreground hover:text-primary transition-colors">Instagram</a>
                </li>
                <li>
                  <a href="#" class="text-muted-foreground hover:text-primary transition-colors">Tiktok</a>
                </li>
              </ul>
            </div>
            <div>
              <h4 class="mb-6 text-xs font-bold tracking-widest uppercase">{{ $t("landing_nav.company") }}
              </h4>
              <ul class="space-y-4 text-sm font-semibold">
                <!-- <li>
                ...
                <li>
                  <NuxtLink to="/privacy-policy" class="text-muted-foreground hover:text-primary transition-colors">{{
                    $t("landing_nav.privacy")
                    }}</NuxtLink>
                </li>
                <li>
                  <NuxtLink to="/terms-of-service" class="text-muted-foreground hover:text-primary transition-colors">{{
                    $t("landing_nav.terms") }}
                  </NuxtLink>
                </li>
              </ul>
